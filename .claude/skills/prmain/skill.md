...
## 흐름
1. 현재 브랜치가 `stg`인지 확인. 아니면 "stg 브랜치에서만 실행할 수 있습니다. 현재 브랜치: <브랜치명>" 출력 후 종료
2. 커밋되지 않은 변경사항이 있으면 "커밋되지 않은 변경사항이 있습니다. 먼저 커밋해주세요." 출력 후 종료
3. `git push -u origin stg` 실행
4. `git log main..HEAD --oneline`으로 포함된 커밋 목록 수집
...
6. `git diff main..HEAD --stat`과 커밋 메시지를 분석하여 실제 변경된 기능에 맞는 테스트 항목 작성
