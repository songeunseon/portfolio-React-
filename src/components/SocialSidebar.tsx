...
export default function SocialSidebar() {
  ...
  const navItems: NavItem[] = [
    ...
    { icon: 'workspace_premium', label: t('nav.certs'), path: '/certifications', hoverColor: 'text-white dark:text-white' },
    ...
    <>
      ...
      <div className="xl:hidden flex justify-center py-4">
        <Link to="/" className="hover:scale-105 transition-transform">
          <img src={logoPng} alt="Home" className="w-40" />
        </Link>
      </div>

      {/* Desktop Logo - Fixed Top Left */}
      <Link to="/" className="hidden xl:block fixed top-6 left-6 z-30 hover:scale-105 transition-transform">
        <img src={logoPng} alt="Home" className="w-50" />
      </Link>
        ...
        {mobileMenuOpen && (
          <>
            ...
            >
              ...
              {/* Theme */}
              <div className="px-5 py-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-end">
                <ThemeSettings />
              </div>
            </motion.div>
          </>
